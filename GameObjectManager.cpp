	...
	if (!haveObject)
	{
		...
		else
		{
			if (pRealEvent->hasCustomOrientation)
			{
				...
				PhysicsComponent* PhysComp = new(HPhys) PhysicsComponent(*m_pContext, m_arena, HPhys, PhysicsComponent::HitBoxType::BOX);
				PhysComp->addDefaultComponents();
				
				m_pContext->getPhysicsManager()->addPhysicsObject(HPhys);
				pSN->addComponent(PhysComp);
				PhysComp->MeshSN = pSN;
				if (pMeshInstance && PhysComp->HBType == PhysicsComponent::HitBoxType::BOX)
				{
					PhysComp->getHitBox();
				}
