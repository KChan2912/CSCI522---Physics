...
SoldierNPC::SoldierNPC(PE::GameContext &context, PE::MemoryArena arena, PE::Handle hMyself, Event_CreateSoldierNPC *pEvt) : Component(context, arena, hMyself)
{
	...
	PE::Handle PhysCompSoldier("PhysicsComponent", sizeof(PhysicsComponent));
	PhysicsComponent* m_PhysCompSoldier = new(PhysCompSoldier) PhysicsComponent(*m_pContext, m_arena, PhysCompSoldier, PhysicsComponent::HitBoxType::SPHERE);
	m_PhysCompSoldier->addDefaultComponents();

	m_pContext->getPhysicsManager()->addPhysicsObject(PhysCompSoldier);
	
	addComponent(PhysCompSoldier);

	m_PhysCompSoldier->getHitBox();

	//Sound Emitter For Soldier
	//emit = m_pContext->getSoundManager()->createEmitter(pMainSN);
